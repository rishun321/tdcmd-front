<template>
  <div class="md-table-pagination">
    ...
    <span>{{ currentItemCount }}-{{ currentPageCount }} {{ mdSeparator }} {{ this.mdTotal }}</span>
...
export default {
  ...
  computed: {
    ...
    currentPageCount () {
      let count = this.mdPage * this.mdPageSize
      return count > this.mdTotal ? this.mdTotal : count
    }
    // ,maxpage () {
    //   console.log('this.mdTotal:' + this.mdTotal)
    //   console.log('this.currentPageSize:' + this.mdPageSize)
    //   let max = Math.round(this.mdTotal / this.mdPageSize)
    //   console.log('max:' + max)
    //   return max
    // }
  },
  ...
  methods: {
    setPageSize () {
      this.$emit('updatePageSize', this.currentPageSize)
    },
